...
class TinyTokenManager {
    ...
    public void Register<T>(int tag, Action<T> action) where T : class, ITinyMessage {
        Register<T>(tag.ToString(), action);
    }
    public void Register<T>(String tag, Action<T> action) where T : class, ITinyMessage {
        tag = typeof(T).ToString() + tag;
    ...
    public void Unregister<T>(int tag) where T : class, ITinyMessage {
        Unregister<T>(tag.ToString());
    }
